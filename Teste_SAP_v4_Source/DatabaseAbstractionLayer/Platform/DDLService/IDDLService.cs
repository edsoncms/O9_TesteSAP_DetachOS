...
namespace OutSystems.HubEdition.Extensibility.Data.Platform.DDLService {
    ...
    // Introduced because of RSAT-643 / RPD-2224 - HACK to avoid breaking change in Oracle when going form TEXT with exactly 2000 characters to anything bigger
    ...
    public interface IDDLService {
        ...
        /// If the column cannot be converted, a <paramref name="outputMessage"/> is also returned.
        /// </summary>
        /// <param name="existingColumnLengthFromOssysEntityAttribute">Existing column length, as declared in OSSYS_ENTITY_ATTR</param>
        ...
        /// <param name="outputMessage">Reason</param>
        /// <returns>True if an alter column operation can be performed, false otherwise</returns>
        // existingColumnLengthFromOssysEntityAttribute parameter introduced because of RSAT-643 / RPD-2224 - HACK to avoid breaking change in Oracle when going form TEXT with exactly 2000 characters to anything bigger
        bool CanAlterColumn(int existingColumnLengthFromOssysEntityAttribute, IPlatformTableSourceColumnInfo existingColumn, IPlatformTableSourceColumnInfo newColumn, out OutputMessage outputMessage);
        ...
        // existingColumnLengthFromOssysEntityAttribute parameter introduced because of RSAT-643 / RPD-2224 - HACK to avoid breaking change in Oracle when going form TEXT with exactly 2000 characters to anything bigger
        IEnumerable<string> AlterColumn(int existingColumnLengthFromOssysEntityAttribute, IPlatformTableSourceColumnInfo existingColumn, IPlatformTableSourceColumnInfo newColumn, string defaultValue);


        /// <summary>
        /// This method validates if one should take special care when changing a TEXT attribute from MAXVARCHAR length to anything bigger.
        /// Introduced by RSAT-643 / RPD-2224 - HACK to avoid breaking change in Oracle when going form TEXT with exactly 2000 characters to anything bigger
        /// </summary>
        /// <param name="existingColumn">Info about the column to change.</param>
        /// <param name="newColumn">New Info about the column.</param>
        /// <returns>True when one needs to take that case into account, False otherwise</returns>
        bool AvoidTextMaxCharsBreakingChange(IPlatformTableSourceColumnInfo existingColumn, IPlatformTableSourceColumnInfo newColumn);
