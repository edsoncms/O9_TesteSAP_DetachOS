...
namespace OutSystems.HubEdition.Extensibility.Data.TransactionService {
    ...
    public class GenericTransactionManager : ITransactionManager {        
        ...
        private const string BubbleUpCommitAndRollbackKey = "OutSystems.Extensibility.Data.TransactionService.BubbleUpErrorOnCommit";

        private bool? bubbleUpCommitAndRollbackSetting = null;
        private bool BubbleUpCommitAndRollbackSetting {
            get {
                if (!bubbleUpCommitAndRollbackSetting.HasValue) {
                    
                    bubbleUpCommitAndRollbackSetting = Convert.ToBoolean(ConfigurationManager.AppSettings[BubbleUpCommitAndRollbackKey]);
                }
                return bubbleUpCommitAndRollbackSetting.Value;
            }
        }

        // add this on machine config to turn it on: <add key="OutSystems.Extensibility.Data.TransactionService.BubbleUpErrorOnCommit" value="true" />
        private bool CheckConnectionCloseException(CloseConnectionTransactionException closeConnectionException) {
            return closeConnectionException != null && BubbleUpCommitAndRollbackSetting;
        }
