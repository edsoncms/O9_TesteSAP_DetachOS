...
namespace OutSystems.HubEdition.RuntimePlatform {
    public class CookieActions {
        ...
        private static RNGCryptoServiceProvider random = null;

        private static object randomLock = new object();

        private static RNGCryptoServiceProvider GetSecureRandom() {
            lock (randomLock) {
                if (random == null) {
        ...
        private static ulong GetRandomLongNumber() {
            ...
            GetSecureRandom().GetBytes(rnd_bytes);
            return BitConverter.ToUInt64(rnd_bytes, 0);
        }

        private static bool UseSessionFixationProtection() {
            return Settings.GetBool(Settings.Configs.EnableSessionFixationProtection);
        }
            ...
            if (!heContext.IsReadOnlySessionRequest) {
                if (Settings.GetBool(Settings.Configs.EnableSessionFixationProtection) && heContext.Session.TerminalType == "WEB") {
                    ...
                    if (protectionValueInSession != null && heContext.PendingSessionProtectionValidation) {
                        ...
                        DateTime cookieExpirationInstant = heContext.Session.SessionFixationProtectionInstant.AddSeconds(Settings.GetInt(Settings.Configs.SessionFixationAllowedOverlapWindowInSecs));
                        if (protectionValueCookie == null || (protectionValueCookie != null && !protectionValueInSession.Equals(protectionValueCookie.Value))) {
                            if (Settings.GetBool(Settings.Configs.DebugSessionFixationProtection)) {
                                GeneralLog.StaticWrite(DateTime.Now, heContext.Session.SessionID, heContext.AppInfo.eSpaceId, 0, heContext.Session.UserId, "Session fixation mismatch with current cookie (session:" + protectionValueInSession + " cookie:" + (protectionValueCookie == null ? "null" : protectionValueCookie.Value) + ")", "INFO", "SESSION", null);
                            }
                            var protectionValueCookieValue = protectionValueCookie == null || protectionValueCookie.Value == null ? "" : protectionValueCookie.Value;
                            if (DateTime.Now < cookieExpirationInstant && heContext.Session.SessionFixationProtectionOld.Equals(protectionValueCookieValue)) {
                                ...
                                if (Settings.GetBool(Settings.Configs.DebugSessionFixationProtection)) {
                                    GeneralLog.StaticWrite(DateTime.Now, heContext.Session.SessionID, heContext.AppInfo.eSpaceId, 0, heContext.Session.UserId, "(repeat) CurrentSessionFixationProtectionCookie (" + heContext.Session.SessionFixationProtection + ")", "INFO", "SESSION", null);
                                }
                                return;
                            }
                            if (Settings.GetBool(Settings.Configs.DebugSessionFixationProtection)) {
            ...
            if (UseSessionFixationProtection()) {
                if (!heContext.IsReadOnlySessionRequest) {
                    if (StringUtils.EqualsIgnoreCase(heContext.Session.TerminalType, "WEB")) {
                        ...
                        heContext.Session.SessionFixationProtectionInstant = DateTime.Now;
                    }
                }
            }
        }
...
#if !JAVA
                case "SMS":
                    return heContext.Session.MSISDN;
#endif
                case "WEB":
                default:
...
#if JAVA
			        // if the value has "'s in the beginning and end, remove it
                    if (StringUtils.StartsWithSegment(cookieValue, "\"") && StringUtils.EndsWithSegment(cookieValue, "\"")) {
