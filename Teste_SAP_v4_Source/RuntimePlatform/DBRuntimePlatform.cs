      ...
      @"SELECT COALESCE(OSSYS_CYCLIC_JOB.SCHEDULE, OSSYS_CYCLIC_JOB_SHARED.SCHEDULE) SCHEDULE
        ...
        public int CreateGeneralLog(DatabaseConnection conn, GeneralLog log, int cycleSize) {
            ...
            using (var cmd = conn.CreateCommand(string.Format(
                "INSERT INTO OSLOG_GENERAL_{0} (INSTANT, SESSION_ID, USER_ID, ESPACE_ID, TENANT_ID, MESSAGE, MESSAGE_TYPE, MODULE_NAME, ERROR_ID, CYCLE, REQUEST_KEY, ENTRYPOINT_NAME, ACTION_NAME) " +
                "VALUES (@INSTANT,@SESSION_ID,@USER_ID,@ESPACE_ID,@TENANT_ID,@MESSAGE,@MESSAGE_TYPE,@MODULE_NAME,@ERROR_ID,@CYCLE,@REQUEST_KEY, @ENTRYPOINT_NAME, @ACTION_NAME)", cycle))) {
                cmd.CreateParameter("@INSTANT", DbType.DateTime, log.Instant);
        ...
        public int CreateScreenLog(DatabaseConnection conn, ScreenLog log, int cycleSize) {
            int cycle = RuntimeLogger.CalculateIndex(log.Instant, cycleSize);
            using (var cmd = conn.CreateCommand(string.Format(
                "INSERT INTO OSLOG_SCREEN_{0} (INSTANT, SESSION_ID, USER_ID, ESPACE_ID, TENANT_ID, SCREEN, DURATION, MSISDN, SCREEN_TYPE, ACCESS_MODE, EXECUTED_BY, CYCLE, SESSION_BYTES, SESSION_REQUESTS, VIEWSTATE_BYTES, REQUEST_KEY, ACTION_NAME) " +
                "VALUES (@INSTANT,@SESSION_ID,@USER_ID,@ESPACE_ID,@TENANT_ID,@SCREEN,@DURATION,@MSISDN,@SCREEN_TYPE,@ACCESS_MODE,@EXECUTED_BY,@CYCLE,@SESSION_BYTES,@SESSION_REQUESTS,@VIEWSTATE_BYTES,@REQUEST_KEY,@ACTION_NAME)", cycle))) {
                cmd.CreateParameter("@INSTANT", DbType.DateTime, log.Instant);
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public void GetEspaceIntegrationsSecurity(Transaction tran, int eSpaceId, out bool IsForcingSecurityForIntegrations) {
            bool dummyBool = false;

            GetEspaceSecurity(tran, eSpaceId, out dummyBool, out IsForcingSecurityForIntegrations, out dummyBool, out dummyBool, out dummyBool);
        }

        [MethodImpl(MethodImplOptions.NoInlining)]
        internal void GetEspaceSecurity(Transaction tran, int eSpaceId, out bool ForceScreenSecurity, out bool ForceIntegrationsSecurity, out bool ForceContentSecurityPolicy, out bool ContentSecurityPolicy_ByApp, out bool ContentSecurityPolicy_ByEnv) {
            int applicationid = -1;
