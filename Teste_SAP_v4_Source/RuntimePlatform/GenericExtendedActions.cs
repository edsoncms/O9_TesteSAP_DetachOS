    ...
    public class GenericExtendedActions {
                ...
                if (!String.IsNullOrEmpty(persistentLoginValue)) {
                    ...
                    using (Transaction trans = DatabaseAccess.ForSystemDatabase.GetRequestTransaction()) {
                        ...
                        if (userId == 0 || existingPersistentLoginId == 0) {
                            ...
                            heContext.AppInfo.InjectionCache.RunCallbacks(info, heContext.Session, Callbacks.CallbackEvent.Login);
        ...
        private static void AttemptLogin(HeContext heContext, int inParamUserId, string inParamPassword, bool inParamPersistent, bool checkPassword) {
            ...
            using (Transaction trans = DatabaseAccess.ForSystemDatabase.GetRequestTransaction()) {
                ...
                string username;
                if (!DBRuntimePlatform.Instance.GetUserInfoForLogin(trans, tenantId, inParamUserId, /*fetchDbHash*/checkPassword, out username, out hashInDb, functionName)) {
            ...
            bool passwordUpgradeOnLogin = checkPassword && isNotServiceCenterTenant
            ...
            if (passwordUpgradeOnLogin || Settings.GetBool(Settings.Configs.Authentication_UpdateUserLastLogin)) {
                try {
                    // update login date
                    // #798465, #1006905 Use a committable transaction to update the last login to avoid locking concurrent logins
                    using (Transaction committableTrans = DatabaseAccess.ForSystemDatabase.GetCommitableTransaction()) {
                        var newDbHash = passwordUpgradeOnLogin ? RuntimePlatformUtils.Hashing.GeneratePasswordHashWithBestAlgorithm(inParamPassword) : null;
                        if (DBRuntimePlatform.Instance.TryUpdateLastLoginAndPasswordIfNeeded(committableTrans, inParamUserId, tenantId, /*upgradePassword*/passwordUpgradeOnLogin, newDbHash, functionName)) {
                            committableTrans.Commit();
                        }
                    }
                } catch (Exception e) {
                    ErrorLog.StaticWrite(DateTime.Now, heContext.SessionID, heContext.AppInfo.eSpaceId, tenantId, inParamUserId, e.Message, e.StackTrace, ErrorLog.GetStackEnvironmentInfo(heContext.AppInfo, heContext), "LOGIN");
                }
            }
        }
    }
