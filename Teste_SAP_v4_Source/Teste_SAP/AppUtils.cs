		...
		public override string getImagePath(bool forInternalAccess, bool includeSessionIdIfNeeded, string nameOverride) {
			...
			try {
				...
				if (!keepPTA || forInternalAccess) {
					path += RuntimePlatformUtils.GetEspacePrettyNamePath(nameOverride ?? "Teste_SAP", heContext);
				} else {
					path += (heContext.OsISAPIFilter.IsActiveInRequest(request) ? heContext.OsISAPIFilter.GetPath(request, "") + "/": "/Teste_SAP/");
				}
				if (keepPTA) {
