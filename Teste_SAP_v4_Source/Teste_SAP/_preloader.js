	...
	function OutSystems_PreloadApp(modules, redirectUrl, locale, ProgressCallback) {	
		...
		function OutSystems_EndPrecache() {
			if(redirectUrl && redirectUrl != '') {
				...
				setTimeout(function(){ window.location.replace(redirectUrl); }, 50); // allow progress bar ui refresh before redirecting the user
			}
			else {
		...
		if(global.outsystems.api.preloader.globalTimeout > 0) {
			setTimeout(function(){ OutSystems_EndPrecache() }, global.outsystems.api.preloader.globalTimeout); // allow progress bar ui refresh before redirecting the user
		}
		...
		modules.forEach(function(entry) {
			...
			xhr.onload = function() {			
				...
				if (xhr.readyState == 4 && xhr.status == 200) {
					var resourceList = xhr.responseText.replace(/\r/g, "").split("\n");
					
					resourceList.forEach(function(resource) {
						if(preloadURLs.urls[resource])
						{
							// ignore entry, resource is already set to be downloaded
						} else {
							preloadURLs.urls[resource] = true; // use an hashtable to remove duplicates
							preloadURLs.size++;
						}
					});
