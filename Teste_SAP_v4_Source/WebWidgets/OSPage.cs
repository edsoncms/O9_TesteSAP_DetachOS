...
namespace OutSystems.HubEdition.WebWidgets {
    public abstract class OSPage : BasePage, IObjectWithViewState {
        ...
        protected virtual void FetchViewState() {
            if (!ViewStateIsValid && Settings.GetBool(Settings.Configs.EncryptViewState)) {
        ...
        public override void DataBind() {
            if (!IsAjaxRequest) {
                ...
                if (Settings.GetBool(Settings.Configs.EnableSecurityFeaturesISA)) {
                    ...
                    if (appInfo.IsForcingContentSecurityPolicy()) {
                        ...
                        var referrer = appInfo.GetReferrer();
                        var report_To = appInfo.GetReportTo();
                        var other = appInfo.GetOtherDirective();

                        if (!report_To.IsEmpty() && report_To.Contains("<internal>")) {

                            var sessionID = AppInfo.GetAppInfo().OsContext.Session.SessionID;
                            var espaceID = appInfo.eSpaceId;
                            var tenantID = appInfo.Tenant.Id;
                            var userID = AppInfo.GetAppInfo().OsContext.Session.UserId;

                            try {

                                var parametersString = espaceID + "," + tenantID + "," + sessionID + "," + userID;
                                ...
                                //eSpaceId, TenantId, SessionId, UserId
                                ...
                                ErrorLog.StaticWrite(new DateTime(), sessionID, espaceID, tenantID, userID, "Could not determine CSP Report-to URL: " + e.Message, e.StackTrace, "CSPReport");
                            }
                        }

                        Response.AddHeader("Content-Security-Policy",
                                (!base_Uri.IsEmpty() ? "base-uri " + base_Uri + "; " : "") +
                                (!child_Src.IsEmpty() ? "child-src " + child_Src + "; " : "") +
                                (!child_Src.IsEmpty() ? "frame-src " + child_Src + "; " : "") +  //Deprecated. Remove when child-src is compatible with firefox.
                                (!connect_Src.IsEmpty() ? "connect-src " + connect_Src + "; " : "") +
                                (!default_Src.IsEmpty() ? "default-src " + default_Src + " 'unsafe-inline' 'unsafe-eval'; " : "") +
                                (!font_Src.IsEmpty() ? "font-src " + font_Src + "; " : "") +
                                (!img_Src.IsEmpty() ? "img-src " + img_Src + "; " : "") +
                                (!media_Src.IsEmpty() ? "media-src " + media_Src + "; " : "") +
                                (!object_Src.IsEmpty() ? "object-src " + object_Src + "; " : "") +
                                (!plugin_Types.IsEmpty() ? "plugin-types " + plugin_Types + "; " : "") +
                                (!script_Src.IsEmpty() ? "script-src " + script_Src + " 'unsafe-inline' 'unsafe-eval'; " : "") +
                                (!style_Src.IsEmpty() ? "style-src " + style_Src + " 'unsafe-inline'; " : "") +
                                (!frame_Ancestors.IsEmpty() ? "frame-ancestors " + frame_Ancestors + "; " : "") +
                                (!referrer.IsEmpty() ? "referrer " + referrer + "; " : "") +
                                (!report_To.IsEmpty() ? "report-uri " + report_To + "; " : "") + //Deprecated. Remove when report-to is compatible with all browsers.
                                (!report_To.IsEmpty() ? "report-to " + report_To + "; " : "") +
                                (!other.IsEmpty() ? other : "")
                                );
        ...
        public void GotoErrorHandlerPage(string pageUrl, string application) {
            if (AppInfo.GetAppInfo().eSpaceName == application) {
                ...
                // #1117522 - the redirect should take into account if request is secure (done by GetRequestUrl)
                string rUrl = "/" + application + "/" + pageUrl;
                var currentUri = new Uri(RuntimePlatformUtils.GetRequestURL(Request));
                rUrl = new Uri(currentUri, rUrl).AbsoluteUri;
        ...
        protected string RenderControl(Control c) {
            using (var w = new StringWriter()) {
                using (var htmlWriter = CreateHtmlTextWriter(w)) {
